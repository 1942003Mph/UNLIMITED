...
@section('content')
@section('H1' , 'All settings')
 <!-- Content Wrapper. Contains page content -->
 <div class="d-flex justify-content-center align-items-center mb-3">
    <a href="{{ route('admin.settings.create') }}" class="btn btn-dark">Add settings</a>
</div>
...
name	image	job	description
    ...
    <tr>
        ...
        <td>{{ $setting->job }}</td>
        <td>@if($setting->status == 1) 
            active 
            @else
             inactive 
            @endif</td>
        <td><img src="{{ asset('uploads/'.$setting->image) }}" width="80" alt=""></td>
        <td>{{ $setting->created_at->format('d F, Y - h:m:s A') }}</td>
        <td>
            <a class="btn btn-primary btn-sm" href="{{ route('admin.settings.edit', $setting->id) }}">
